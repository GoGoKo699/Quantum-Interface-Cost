...
Updated: 2026-09-24. Stage: the [two-sharp-pair converse](audits/TWO_SHARP_PAIR_CONVERSE.md) proves `||H||<=4+sqrt(2)` whenever two original ququart query pairs are sharp and internally anticommuting, with no cross-commutation restriction and an arbitrary third binary-POVM pair. The proof combines a sharp Schmidt-rank-two score cap, actual top-state tail constraints, the stronger spectral circle `(U-2)^2+(m-2)^2<=4`, and the preceding exact last-query resolvent. An analytical reduction leaves one scalar variable; a 400-cell rational interval certificate closes its entire domain. Both earlier synthetic obstructions are excluded. Arbitrary earlier Jordan angles, all three complete reflection signatures, unrestricted equal-accuracy optimality and publication originality remain unresolved. The prior nonlinear-CHSH subsumption remains in force.
Latest research base: `9fc2be03587986d85341aa3f766653e6ba94bb75`, the merge of PR #42.
