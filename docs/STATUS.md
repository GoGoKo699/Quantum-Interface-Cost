...
Updated: 2026-09-24. Stage: the [noncommuting-family continuation](audits/NONCOMMUTING_QUERY_FAMILIES.md) proves two further continuous-family converses. Two sharp internally anticommuting query pairs need only two mixed cross commutators to vanish, with the third pair unrestricted. Independently, three partial-SWAP Bell-subspace projectors obey the sharp sum bound 5/2 with all equality cases classified. Equal spectra within each site's two Jordan blocks then imply the retention benchmark for that geometry. A strict unrestricted improvement still requires a nonflat rank-four seed, at least two double-block optimal pairs, and all three query algebras pairwise noncommuting. Exactly three complete signature patterns remain open. Unrestricted equal-accuracy optimality and publication originality remain unresolved; the prior nonlinear-CHSH subsumption remains in force.
Latest research base: `e8fffa3ccac4b03b354d354101e20302fc934616`, the merge of PR #38.
