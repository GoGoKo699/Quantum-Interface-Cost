...
Updated: 2026-09-26. Stage: the [two-mode resolvent reduction](audits/TWO_MODE_RESOLVENT.md) identifies the smallest universally sufficient spectral head for a safe last-query resolvent. For arbitrary earlier binary POVMs on four-dimensional memory, the third eigenvalue obeys `lambda_3(H0)<=sqrt(32/3)<2+sqrt(2)`. Retaining two top modes therefore always gives a positive resolvent denominator and an exact 4-by-4 test for the resulting spectral envelope. An explicit physical family with `U=m=2sqrt(3)` proves that one mode cannot work universally. Its exact head channel shows why even flat individual marginals do not permit dropping cross-mode coherences. The new matrix test has not been proved for every physical head channel; the unrestricted converse remains open. The preceding [two-sharp-pair converse](audits/TWO_SHARP_PAIR_CONVERSE.md) remains proved. Arbitrary earlier Jordan angles, all three complete reflection signatures, unrestricted equal-accuracy optimality and publication originality remain unresolved.
Latest research base: `8a88536b62a3e9ac2082b1108416af7958d87dda`, the merge of PR #43.
...
| Complete joint-data representation for two sharp anticommuting ququart pairs | Derived and independently reconstructed from established Cartan decomposition: seven parameters and an eight-dimensional singular-value problem preserve the full spectrum and actual top memory marginal; no converse follows without the remaining inequality | audits/JOINT_SPECTRUM_NORMAL_FORM.md Sections 1–4 |
| Two sharp query pairs have exact score cap 3 on every state of Schmidt rank at most two across references versus memory | Derived and independently checked; an elementary qubit Choi bound with fixed shared-memory marginal supplies the global cap. Every pure attainer retains one Bell pair and a pure X/Z-plane state on the other reference; this classifies the occupied support, not the full readout algebras | audits/SHARP_PAIR_SCHMIDT_TAIL.md |
| Actual sharp-pair top eigenvectors obey lambda_3+lambda_4>=max(0,(U-3)/(U+m)) and lambda_1<=(2+m)/(U+m) | Derived and independently checked by chiral singular-vector compression; the subsequent two-sharp-pair converse combines these with the spectral circle and six-Pauli condition to prove the resolvent certificate | audits/SHARP_PAIR_SCHMIDT_TAIL.md; audits/TWO_SHARP_PAIR_CONVERSE.md |
| Two sharp query pairs obey the sharp spectral circle (U-2)^2+(m-2)^2<=4 | Derived and independently checked from the fixed positive-projector overlap. Strictly stronger than the prior spectral-sum bound; it excludes both exact synthetic obstructions examined so far | audits/SHARP_PAIR_SCHMIDT_TAIL.md |
| Separate spectral and known fixed-spectrum score constraints suffice for the rank-one resolvent test | Incorrect even with full-rank spectra, SLD and the stronger six-Pauli condition; an exact synthetic tuple fails the test. No Hamiltonian realizing that tuple is supplied | audits/JOINT_SPECTRUM_NORMAL_FORM.md Sections 5–6 |
| Sharp-pair spectral-sum bound U+m<=4+2sqrt(2) | The prior upper bound is attained by an explicit Pauli construction at U=m=2+sqrt(2); decreasing its universal constant cannot repair the separate-data relaxation | audits/JOINT_SPECTRUM_NORMAL_FORM.md Section 7 |
