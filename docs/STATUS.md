...
Updated: 2026-09-24. Stage: the [one-double-block converse](audits/SINGLE_DOUBLE_BLOCK_CONVERSE.md) closes three more complete ququart signature patterns, leaving exactly three unresolved. Any strict three-input/two-qubit advantage now requires a nonflat rank-four seed and at least two optimal query pairs with two noncommuting Jordan blocks. The proof retains the exceptional pair's positive spectrum and combines two compressed monogamy bounds; the required mixed-Choi bound is an established-theorem corollary. The prior all-block scalar certificate excludes additional regions within each remaining pattern. Unrestricted equal-accuracy optimality and publication originality remain open; the prior nonlinear-CHSH subsumption remains in force.
Latest research base: `ac5863e1e026a8a9432a27385e6ee9491b423e92`, the merge of PR #36.
