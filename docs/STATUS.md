...
Updated: 2026-09-24. Stage: the [separate-query converse](audits/COMMUTING_QUERY_ALGEBRAS.md) proves the exact retention optimum at every n=q+1 when q query pairs address distinct memory qubits and the last pair is unrestricted. Arbitrary binary POVMs and collective encoders are covered. At three inputs, any two commuting original query algebras suffice to rule out an advantage. A strict improvement therefore requires a nonflat rank-four seed, at least two double-block optimal pairs, and all three query algebras pairwise noncommuting. Exactly three complete signature patterns remain open, with additional regions excluded by the prior scalar certificate. Unrestricted equal-accuracy optimality and publication originality remain unresolved; the prior nonlinear-CHSH subsumption remains in force.
Latest research base: `a383214b357ccbd83a62ae0c94660083cb8b090e`, the merge of PR #37.
